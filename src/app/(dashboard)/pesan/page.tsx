...
const Avatar: FC<{ name: string; uid: string; size?: 'sm' | 'md' }> = ({ name, uid, size = 'md' }) => {
    ...
    getDocs(query(collection(db, 'users'), where('role', 'in', ['teacher', 'admin'])))
      .then(snap => {
        setContacts(snap.docs.map(d => ({
          uid: d.id,
          displayName: d.data().displayName || 'Guru',
          role: d.data().role,
        })));
      })
      .catch(() => { })
      ...
      <div className="flex w-72 shrink-0 flex-col border-r border-gray-100">
        ...
        <div className="flex-1 overflow-y-auto px-2 pb-3 space-y-0.5">
          {filteredContacts.length === 0 && (
            <p className="px-3 py-6 text-center text-xs text-gray-400">Tidak ada guru ditemukan</p>
          )}
          {filteredContacts.map(contact => (
            <button
              key={contact.uid}
              onClick={() => setSelectedUid(contact.uid)}
              className={`flex w-full items-center gap-3 rounded-xl px-3 py-2.5 text-left transition-all ${selectedUid === contact.uid ? 'bg-violet-50' : 'hover:bg-gray-50'}`}
            >
          ...
          {!selectedUid ? (
            <motion.div key="empty" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}
              ...
              <div className="text-center">
                ...
                <p className="mt-1 text-xs text-gray-400">Pilih guru di sebelah kiri untuk mulai chat</p>
              </div>
            </motion.div>
          ) : (
            <motion.div key={selectedUid} initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}
              className="flex flex-1 flex-col min-h-0">
              {/* Header */}
              <div className="flex items-center gap-3 border-b border-gray-100 px-5 py-3.5">
                {selectedContact && (
                  <>
                    <Avatar name={selectedContact.displayName} uid={selectedContact.uid} />
                    <div>
                      <p className="text-sm font-semibold text-gray-900">{selectedContact.displayName}</p>
                      <p className="text-[11px] text-gray-400 capitalize">{selectedContact.role}</p>
                    </div>
                  </>
                )}
              </div>
                        ...
                        return (
                          ...
                          >
                            ...
                            <div className={`flex flex-col gap-0.5 max-w-[65%] ${isMine ? 'items-end' : 'items-start'}`}>
                              <div className={`px-4 py-2.5 text-sm leading-relaxed break-words text-white ${isMine ? `${senderBubble} ${myRadius}` : `${senderBubble} ${theirRadius}`}`}>
                                {msg.content}
                              </div>
                              {isLastInChain && (
              ...
              <div className="border-t border-gray-100 bg-white px-4 py-3">
                <div className="flex items-center gap-2">
                  ...
                  <button
                    ...
                    className="flex h-11 w-11 shrink-0 items-center justify-center rounded-full bg-violet-600 text-white shadow-sm transition-all disabled:opacity-40 hover:enabled:bg-violet-700 active:enabled:scale-95"
                  >
