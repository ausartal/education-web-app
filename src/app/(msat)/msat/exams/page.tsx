...
import {
  ClipboardCheck, Plus, RefreshCw, Loader2, Copy, Check,
  Users, Clock, ChevronRight, Settings2, Play, Square,
} from 'lucide-react';
...
const MSATExamsPage: FC = () => {
  ...
  const fetchExams = useCallback(async () => {
    ...
    try {
      ...
      addToast('error', 'Gagal memuat daftar ujian MSAT');
    } finally {
      setLoading(false);
    }
  }, [user, addToast]);
  ...
  const handleEnd = async (examId: string) => {
    if (!user || !confirm('Akhiri ujian ini?')) return;
    try {
      const token = await user.getIdToken();
      const res = await fetch(`/api/admin/mast-exams/${examId}/end`, {
        method: 'POST',
        headers: { Authorization: `Bearer ${token}` },
      });
      if (!res.ok) throw new Error('Gagal mengakhiri');
      addToast('success', 'Ujian berhasil diakhiri');
      fetchExams();
    } catch {
      addToast('error', 'Gagal mengakhiri ujian');
    }
  };
    ...
    <div className="space-y-5">
      ...
      <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} className="flex items-center justify-between">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h1 className="font-display text-2xl font-extrabold text-gray-900">Ujian MSAT</h1>
            <p className="text-sm text-gray-500">Kelola ujian adaptif bertingkat</p>
          </div>
        </div>
                ...
                return (
                  <motion.tr key={exam.id} initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: i * 0.03 }}
                    ...
                    <td className="px-5 py-3.5">
                      <Link href={`/msat/exams/${exam.id}`} className="group">
                        <p className="font-semibold text-gray-900 group-hover:text-indigo-700 transition-colors">{exam.title}</p>
                        <p className="text-[10px] text-gray-400 mt-0.5 line-clamp-1">{exam.description}</p>
                      </Link>
                    </td>
                    <td className="px-3 py-3.5">
                      <div className="flex items-center gap-1.5">
                        <code className="rounded-lg bg-indigo-50 px-2 py-1 font-mono text-[11px] font-bold text-indigo-700">{exam.examCode}</code>
                        <button onClick={() => copyCode(exam.examCode, exam.id)} className="text-gray-300 hover:text-indigo-500 transition-colors">
                          {copiedId === exam.id ? <Check size={12} className="text-emerald-500" /> : <Copy size={12} />}
                        </button>
                      </div>
                    </td>
                    <td className="px-3 py-3.5">
                      <span className={`rounded-full px-2.5 py-0.5 text-[10px] font-semibold ${modeCfg.color}`}>{modeCfg.label}</span>
                    </td>
                    <td className="px-3 py-3.5">
                      <span className={`rounded-full px-2.5 py-0.5 text-[10px] font-semibold ${statusCfg.color}`}>{statusCfg.label}</span>
                    </td>
                    <td className="px-3 py-3.5">
                      <div className="flex items-center gap-1.5">
                    ...
                    <td className="px-3 py-3.5 text-[10px] text-gray-400 whitespace-nowrap">{fmtDate(exam.createdAt)}</td>
                    <td className="px-3 py-3.5">
                      <div className="flex items-center justify-end gap-1">
                        {exam.status === 'draft' && (
                          <button onClick={() => handleActivate(exam.id)}
                            className="flex items-center gap-1 rounded-lg px-2 py-1.5 text-[11px] font-medium text-emerald-600 hover:bg-emerald-50 transition-colors">
                            <Play size={11} /> Aktifkan
                          </button>
                        )}
                        {(exam.status === 'active' || exam.status === 'in_progress') && (
                          <button onClick={() => handleEnd(exam.id)}
                            className="flex items-center gap-1 rounded-lg px-2 py-1.5 text-[11px] font-medium text-rose-600 hover:bg-rose-50 transition-colors">
                            <Square size={11} /> Akhiri
                          </button>
                        )}
                        <Link href={`/msat/exams/${exam.id}`}
                ...
                <tr>
                  <td colSpan={7} className="py-16 text-center">
                    <div className="flex flex-col items-center gap-3">
                      <ClipboardCheck size={28} className="text-stone-300" />
                      <p className="text-sm font-semibold text-gray-400">Belum ada ujian MSAT</p>
                      <Link href="/msat/exams/create"
                        className="mt-2 flex items-center gap-2 rounded-xl bg-indigo-600 px-4 py-2.5 text-sm font-semibold text-white hover:bg-indigo-700">
                        <Plus size={15} /> Buat Ujian Baru
                      </Link>
                    </div>
                  </td>
      ...
      {exams.length > 0 && (
        <motion.div initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.2 }}
          ...
          {[
            { label: 'Total Ujian', value: exams.length, color: 'text-indigo-600', bg: 'bg-indigo-50' },
            { label: 'Draft', value: exams.filter(e => e.status === 'draft').length, color: 'text-amber-600', bg: 'bg-amber-50' },
            { label: 'Berlangsung', value: exams.filter(e => e.status === 'in_progress' || e.status === 'active').length, color: 'text-blue-600', bg: 'bg-blue-50' },
            { label: 'Total Peserta', value: exams.reduce((sum, e) => sum + (e.participantCount ?? 0), 0), color: 'text-emerald-600', bg: 'bg-emerald-50' },
          ].map(s => (
