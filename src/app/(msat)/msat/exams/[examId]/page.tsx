...
import {
  ...
  Users, BarChart3, Coffee, Trophy, RefreshCw, ClipboardCheck,
} from 'lucide-react';
  ...
  if (!exam) {
    return (
      <div className="flex min-h-[60vh] flex-col items-center justify-center gap-4">
        ...
        <Link href="/msat/exams" className="text-sm font-semibold text-indigo-600">Kembali ke daftar</Link>
      </div>
    );
  }
  ...
  return (
    <div className="space-y-5">
        ...
        <div className="flex items-center gap-2">
          ...
          {exam.status === 'draft' && (
            <button onClick={handleActivate} disabled={activating}
              ...
              {activating ? <Loader2 size={14} className="animate-spin" /> : <Play size={14} />} Aktifkan
            </button>
          )}
          <Link href={`/msat/exams/${examId}/edit`}
      ...
      <div className="grid grid-cols-2 gap-3 lg:grid-cols-4">
        {[
          { label: 'Kode Ujian', content: (
            <div className="flex items-center gap-2">
              <code className="font-mono text-xl font-bold tracking-widest text-indigo-700">{exam.examCode}</code>
              <button onClick={copyCode} className="text-gray-300 hover:text-indigo-500 transition-colors">
          ...
          { label: 'Mode', content: (
            <>
              <span className={`rounded-full px-2.5 py-0.5 text-xs font-semibold ${modeCfg.color}`}>{modeCfg.label}</span>
              <p className="mt-1 text-[10px] text-gray-400">{exam.mode === 'auto_start' ? 'Langsung mulai' : 'Menunggu admin'}</p>
            </>
          )},
          { label: 'Durasi', content: (
            <>
          ...
          <motion.div key={card.label} initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.05 + i * 0.05 }}
            className="rounded-2xl bg-white p-5 shadow-sm border border-stone-100">
            <p className="mb-2 text-[10px] font-bold uppercase tracking-wide text-gray-400">{card.label}</p>
            {card.content}
          </motion.div>
        ))}
      </div>
      ...
      <motion.div initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.25 }}
        ...
        <h2 className="mb-4 text-sm font-bold text-gray-900">Statistik</h2>
        <div className="grid grid-cols-3 gap-4">
          ...
          <div className="rounded-xl bg-stone-50 p-4 text-center">
            ...
            <p className="font-display text-2xl font-black text-gray-900">
              {exam.avgScore != null ? exam.avgScore.toFixed(1) : '—'}
            </p>
      ...
      <motion.div initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.3 }}
        ...
        {[
          { title: 'Ruang Tunggu', desc: 'Monitor siswa dan mulai ujian', icon: Users, href: `/msat/exams/${examId}/waiting-room`, color: 'text-blue-600', bg: 'bg-blue-50', enabled: exam.mode === 'manual_start' },
          { title: 'Monitor Istirahat', desc: 'Pantau siswa yang sedang istirahat', icon: Coffee, href: `/msat/exams/${examId}/breaks`, color: 'text-amber-600', bg: 'bg-amber-50', enabled: true },
          { title: 'Rekap Hasil', desc: 'Skor, predikat, dan analisis', icon: Trophy, href: `/msat/exams/${examId}/results`, color: 'text-emerald-600', bg: 'bg-emerald-50', enabled: true },
        ].map(item => {
          ...
          return (
            <Link key={item.title} href={item.href}
              ...
              <div className="flex items-start justify-between">
                <div className={`rounded-xl p-2.5 ${item.bg}`}><Icon size={18} className={item.color} /></div>
                <span className="text-xs text-gray-400 group-hover:text-indigo-600 transition-colors">Buka →</span>
              </div>
              <h3 className="mt-3 text-sm font-bold text-gray-900">{item.title}</h3>
              <p className="mt-1 text-xs text-gray-500">{item.desc}</p>
              {!item.enabled && <p className="mt-2 text-[10px] font-medium text-amber-600">Hanya untuk mode manual</p>}
            </Link>
          );
        })}
      </motion.div>
