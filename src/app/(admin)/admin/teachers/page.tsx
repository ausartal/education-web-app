    ...
    <tr className="bg-emerald-50/40">
      <td colSpan={8} className="px-8 py-4">
        <div className="grid grid-cols-2 gap-6 lg:grid-cols-4">
          <div>
            <p className="mb-1 text-[10px] font-bold uppercase tracking-wide text-stone-500">Materi Dibuat</p>
            <div className="flex items-center gap-2">
              <FileText size={14} className="text-blue-500" />
              <span className="text-lg font-black text-stone-900">{t.materialsCount}</span>
            </div>
          </div>
          <div>
            <p className="mb-1 text-[10px] font-bold uppercase tracking-wide text-stone-500">Soal Latihan</p>
            <div className="flex items-center gap-2">
              <BookOpen size={14} className="text-amber-500" />
              <span className="text-lg font-black text-stone-900">{t.questionsCount}</span>
            </div>
          </div>
          <div>
            <p className="mb-1 text-[10px] font-bold uppercase tracking-wide text-stone-500">Kelas Aktif</p>
            <div className="flex items-center gap-2">
              <School size={14} className="text-cyan-500" />
              <span className="text-lg font-black text-stone-900">{t.classesCount}</span>
              <span className="text-xs text-stone-400">({t.studentsCount} siswa)</span>
            </div>
          </div>
          <div>
            <p className="mb-1 text-[10px] font-bold uppercase tracking-wide text-stone-500">Jadwal Ujian</p>
            <div className="flex items-center gap-2">
              <CalendarCheck size={14} className="text-violet-500" />
              <span className="text-lg font-black text-stone-900">{t.schedulesCount}</span>
            </div>
          </div>
          <div>
            <p className="mb-1 text-[10px] font-bold uppercase tracking-wide text-stone-500">Total Sesi Ujian</p>
            <div className="flex items-center gap-2">
              <ClipboardList size={14} className="text-fuchsia-500" />
              <span className="text-lg font-black text-stone-900">{t.sessionsTotal}</span>
              <span className="text-xs text-stone-400">({t.sessionsCompleted} selesai)</span>
            </div>
          </div>
          <div>
            <p className="mb-1 text-[10px] font-bold uppercase tracking-wide text-stone-500">Tingkat Penyelesaian</p>
          ...
          <div className="col-span-2">
            <p className="mb-1 text-[10px] font-bold uppercase tracking-wide text-stone-500">Navigasi Cepat</p>
...
const AdminTeachers: FC = () => {
  ...
  const handlePromoteToAdmin = async (uid: string, name: string) => {
    if (!user) return;
    if (!(await confirmAction({ title: 'Promosikan menjadi admin?', description: `${name} akan memperoleh akses administratif penuh pada model permission saat ini.`, confirmLabel: 'Promosikan', tone: 'warning' }))) return;
    try {
      const token = await user.getIdToken();
      const res = await fetch(`/api/admin/users/${uid}`, {
        method: 'PATCH',
    ...
    <div className="space-y-5">
      ...
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h1 className="font-display text-2xl font-extrabold text-stone-900">Manajemen Guru</h1>
            <p className="text-sm text-stone-500">{teachersOnly.length} guru terdaftar</p>
          </div>
        </div>
        <button onClick={fetchTeachers}
          className="flex items-center gap-1.5 rounded-xl border border-stone-100 bg-white px-3 py-2 text-xs font-medium text-stone-600 shadow-sm hover:bg-stone-50">
      ...
      <div className="grid grid-cols-2 gap-3 lg:grid-cols-5">
        {[
          { label: 'Total Guru', value: teachersOnly.length, color: 'text-stone-900', icon: GraduationCap },
          ...
          const Icon = s.icon;
          return (
            <div key={s.label} className="rounded-2xl border border-stone-100 bg-white p-4 shadow-sm">
                    ...
                    <td className="px-5 py-3">
                      <div className="flex items-center gap-3">
                        ...
                        <div>
                          ...
                          <p className="text-[10px] text-stone-400">Bergabung {fmtDate(t.createdAt)}</p>
                        </div>
                      </div>
                    </td>
                    <td className="px-4 py-3">
                      <span className={`rounded-full px-2.5 py-1 text-xs font-semibold ${t.isActive ? 'bg-emerald-50 text-emerald-700' : 'bg-stone-100 text-stone-500'}`}>
                        {t.isActive ? 'Aktif' : 'Nonaktif'}
                      </span>
                    </td>
                    <td className="px-4 py-3 text-center">
                      <span className={`font-bold ${t.materialsCount > 0 ? 'text-blue-600' : 'text-stone-300'}`}>
                        {t.materialsCount}
                      </span>
                    </td>
                    <td className="px-4 py-3 text-center">
                      <span className={`font-bold ${t.questionsCount > 0 ? 'text-amber-600' : 'text-stone-300'}`}>
                        {t.questionsCount}
                      </span>
                    </td>
                    <td className="px-4 py-3 text-center">
                      <span className={`font-bold ${t.classesCount > 0 ? 'text-cyan-600' : 'text-stone-300'}`}>
                        {t.classesCount}
                      </span>
                    </td>
                    <td className="px-4 py-3 text-center">
                      <span className={`font-bold ${t.studentsCount > 0 ? 'text-blue-600' : 'text-stone-300'}`}>
                        {t.studentsCount}
                      </span>
                    </td>
                    <td className="px-4 py-3 text-center">
                      <div className="flex flex-col items-center">
                        <span className={`font-bold ${t.sessionsTotal > 0 ? 'text-fuchsia-600' : 'text-stone-300'}`}>
                          {t.sessionsTotal}
                        </span>
                        {t.sessionsTotal > 0 && (
                          <span className="text-[9px] text-stone-400">{t.sessionsCompleted} selesai</span>
                        )}
                      </div>
                    </td>
                    <td className="px-4 py-3">
                      <div className="flex items-center gap-1" onClick={e => e.stopPropagation()}>
                        <button onClick={() => handleDeactivate(t.uid, t.isActive)}
                          className="rounded-lg p-2 text-stone-400 transition-colors hover:bg-amber-50 hover:text-amber-600"
                          title={t.isActive ? 'Nonaktifkan' : 'Aktifkan'}>
                          <UserX size={14} />
                        </button>
                        {t.role !== 'admin' && (
                          <button onClick={() => handlePromoteToAdmin(t.uid, t.displayName)}
                            className="rounded-lg p-2 text-stone-400 transition-colors hover:bg-violet-50 hover:text-violet-600"
